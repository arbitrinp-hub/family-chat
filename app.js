...
async function login() {
  ...
  if (!existing || existing.length === 0) {
    ...
    if (inv && !inv.approved) {
      await sb.from('users').update({ name, initials: ini, color, approved: true, invite_code: null }).eq('invite_code', code);
  ...
  const { data: user, error } = await sb.from('users').insert({ id, name, initials: ini, color, approved, online: true }).select().single();
...
async function createInvite() {
  ...
  await sb.from('users').insert({ id, name: 'Ожидает...', initials: '?', color: 'gray', approved: false, invite_code: code, online: false });
...
function updateOnline(online) {
  if (!ME) return;
  sb.from('users').update({ online, last_seen: new Date().toISOString() }).eq('id', ME.id).then(() => {});
}
...
function renderMembers() {
  ...
  MEMBERS.filter(m => m.id !== ME?.id).forEach(m => {
    ...
    const seen = m.online ? '<span style="color:var(--green)">онлайн</span>' : formatTime(m.last_seen);
    info.innerHTML = `<div class="chat-name">${m.name}</div><div class="chat-preview">${seen}</div>`;
...
function renderChatList() {
  ...
  groupRow.innerHTML = `<div class="av av-teal">👨‍👩‍👧</div><div class="chat-info"><div class="chat-name">Общий чат</div><div class="chat-preview">${MEMBERS.length} участников</div></div>`;
  wrap.appendChild(groupRow);
  MEMBERS.filter(m => m.id !== ME?.id).forEach(m => {
    const row = el('div', 'chat-row');
    row.onclick = () => openChat(m.id);
    row.innerHTML = `<div class="av av-${m.color}">${m.initials}</div><div class="chat-info"><div class="chat-name">${m.name}</div><div class="chat-preview" style="color:${m.online ? 'var(--green)' : 'var(--text2)'}">${m.online ? 'онлайн' : formatTime(m.last_seen)}</div></div>`;
    wrap.appendChild(row);
  });
...
async function openChat(userId) {
  ...
  const sub = isGroup ? `${MEMBERS.length} участников` : (member?.online ? '<span style="color:var(--green)">онлайн</span>' : formatTime(member?.last_seen));
  const avHtml = isGroup ? `<div class="av av-teal">👨‍👩‍👧</div>` : `<div class="av av-${member?.color}">${member?.initials}</div>`;
  tb.innerHTML = `<button class="back-btn" onclick="go('home')" aria-label="Назад"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" style="width:22px;height:22px"><polyline points="15 18 9 12 15 6"/></svg></button>${avHtml}<div style="flex:1"><div class="topbar-title">${name}</div><div class="topbar-sub">${sub}</div></div>`;
  go('chat');
...
function appendMessage(msg, scroll = true) {
  ...
  if (!isOut && isGroup && sender) row.appendChild(el('div', `av av-${sender.color} sm`, sender.initials));
  const bwrap = el('div');
  if (!isOut && isGroup && sender) bwrap.innerHTML = `<div class="msg-sender">${sender.name}</div>`;
  if (msg.file_url) {
...
async function uploadFile(file) {
  ...
  const cid = chatId(activeChatId);
  await sb.from('messages').insert({ chat_id: cid, from_id: ME.id, file_url: urlData.publicUrl, file_type: file.type, file_name: file.name });
}
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  if (saved) {
    ...
    if (data?.approved) { ME = data; startApp(); return; }
    else if (data && !data.approved) { go('pending'); return; }
  }
  go('login');
  checkIOSInstall();
