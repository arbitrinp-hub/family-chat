...
async function loadMembers() {
  const { data } = await sb.from('users').select('*').neq('id', ME?.id || '');
  MEMBERS = data || [];
  renderChatList();
}
...
function renderMembers() {
  ...
  MEMBERS.forEach(m => {
    ...
    info.innerHTML = `<div class="chat-name">${m.name}</div>
      <div class="chat-preview" style="color:${m.approved ? '' : '#e53e3e'}">${m.approved ? seen : '⏳ Ожидает одобрения'}</div>`;
    row.appendChild(avWrap);
    row.appendChild(info);
    if (!m.approved) {
      const approveBtn = el('button', '');
      approveBtn.style.cssText = 'background:var(--green);color:#fff;border:none;border-radius:8px;padding:6px 12px;font-size:13px;cursor:pointer;flex-shrink:0';
      approveBtn.textContent = '✓ Принять';
      approveBtn.onclick = async () => {
        await sb.from('users').update({ approved: true }).eq('id', m.id);
        loadMembers();
      };
